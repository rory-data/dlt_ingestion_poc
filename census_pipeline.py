...
from helpers.duckdb import get_duckdb_data_arrow
from helpers.standardise import clean_data
...
def census_source() -> Iterator[pa.Table]:
    ...
    yield get_duckdb_data_arrow(
        db_path="files/input/census.ddb",
...
def census_clean(raw_data_chunk: pa.Table) -> Iterator[pa.Table]:  # Corrected input type hint
    """
    ...
    """
    logger.debug(f"Received Arrow Table chunk with {raw_data_chunk.num_rows} records for cleaning.")
...
if __name__ == "__main__":
    ...
    pipeline = dlt.pipeline(
        ...
        destination=filesystem(bucket_url="files/output"),

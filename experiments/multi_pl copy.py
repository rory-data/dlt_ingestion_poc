...
def scan_and_filter_record_type(file_path: str, record_type: str) -> pl.LazyFrame:
    ...
    return (
        ...
        .select(pl.col("raw_line").str.split("|").alias("fields"))
    )


def compute_schema_for_record_type(file_path: str, record_type: str) -> int:
    ...
    max_fields_df = (
        lazy_df.select(pl.col("fields").list.len().alias("field_count"))
...
def extract_fields_to_columns(df: pl.DataFrame, num_fields: int) -> pl.DataFrame:
    ...
    """
    return df.select(
        [pl.col("fields").list.get(i).alias(f"field_{i}") for i in range(num_fields)]
    )
            ...
            try:
                ...
                if "field_0" in table.column_names:
                    ...
                    if unique_values != {record_type}:
                        ...
                        logger.success("    ✓ field_0 validation passed")
                else:
                    errors.append("field_0 column not found")
                    logger.error("    ✗ field_0 column not found")
